		...
		<tr hidden={pageCount === 1}>
			<td colSpan={columns.length} className='px-2 border-t border-gray-300'>
				<div className='flex justify-end items-center p-1 space-x-1'>
					...
					{Array.from(Array(pageCount).keys()).map((e: number) => <button key={e} onClick={() => setPage(e)} className={`p-2 rounded-md text-base ${e === page ? 'bg-gray-300' : 'hover:bg-gray-100 active:bg-gray-300 hover:text-gray-900'}`}><div className='block w-5 h-5'>{e + 1}</div></button>)}
